...
import { Plus, X, ArrowRight, ArrowLeft, Calendar as CalendarIcon, DollarSign, Tag, FileText } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
import type { SavingsEntry } from "@/lib/types"

interface AddSavingsPanelProps {
  onAdd: (entry: Omit<SavingsEntry, 'id'>) => void
}

export function AddSavingsPanel({ onAdd }: AddSavingsPanelProps) {
  ...
  return (
    <>
      ...
      <button
        ...
        className="group inline-flex items-center justify-center rounded-full text-sm font-medium ring-offset-background transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-primary text-primary-foreground hover:bg-primary/90 hover:scale-110 active:scale-95 h-16 w-16 shadow-lg hover:shadow-2xl"
      >
        <Plus className="h-7 w-7 transition-transform duration-300 group-hover:rotate-90" />
        <span className="sr-only">Add Savings</span>
      </button>
      ...
      {/* Slide-out Panel */}
      <div
        className={`fixed right-0 top-0 z-50 h-full w-full sm:w-[500px] bg-background border-l shadow-2xl transition-transform duration-300 ease-out ${
                ...
                <div className="space-y-4">
                  <div className="relative">
                    ...
                    <Input
                      ...
                      className="h-16 pl-14 text-2xl font-semibold"
                      autoFocus
                    />
                  </div>
                  <div className="grid grid-cols-3 gap-2">
                    {['10', '50', '100', '200', '500', '1000'].map((preset) => (
                      <button
                        ...
                        className="rounded-lg border bg-muted/50 px-4 py-2 text-sm font-medium hover:bg-muted transition-colors"
                      >
                        ${preset}
                      </button>
                    ))}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {['Emergency Fund', 'Vacation', 'New Car', 'Home Down Payment', 'Retirement', 'Education'].map((preset) => (
                        <button
                          ...
                          className="rounded-full border bg-muted/50 px-4 py-1.5 text-sm font-medium hover:bg-muted transition-colors"
                        >
                          {preset}
                        </button>
                      ))}
            ...
            {step === 4 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                ...
                <div className="space-y-4">
                  ...
                  {/* Summary Card */}
                  <div className="mt-6 rounded-2xl border-2 border-dashed bg-muted/50 p-6 space-y-3">
